...
const TechChip = ({ name }: { name: string }) => {
  ...
  } else if (n.includes("type")) {
    icon = <SiTypescript />;
    color = "text-blue-600";
    bg = "bg-blue-50";
    border = "border-blue-200";
  } else if (n.includes("js")) {
    icon = <SiJavascript />;
  ...
  return (
    <div
      className={`relative group/chip flex items-center justify-center w-9 h-9 rounded-lg border ${border} ${bg} shadow-sm transition-all hover:scale-110 hover:shadow-md cursor-help`}
    >
        ...
        <div className="relative w-full aspect-video overflow-hidden bg-slate-100 group/image">
          <div className="absolute top-4 right-4 z-30">
            <div className="relative w-12 h-12 flex items-center justify-center">
              ...
              <div
                className={`absolute inset-1 rounded-lg rotate-45 opacity-80 ${rank === "S" ? "bg-linear-to-br from-amber-300 to-amber-500" : "bg-linear-to-br from-indigo-400 to-indigo-600"}`}
              />
              <span className="relative z-10 font-heading font-black text-white text-xl drop-shadow-md">
                {rank}
              </span>
            </div>
          </div>
        ...
        <div className="relative z-10 flex flex-col grow p-6 bg-linear-to-b from-white to-slate-50/30">
          <div className="mb-4 flex items-center justify-between">
            ...
            <span className="text-[10px] font-bold text-slate-300 font-mono tracking-wider">
              PROJECT_ID_0{Math.floor(Math.random() * 9)}
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-6">
            {stats.map((stat, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between px-3 py-2 bg-white border border-slate-100 rounded-lg shadow-sm group/stat hover:border-indigo-100 transition-colors"
              >
                <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider flex items-center gap-1">
                  {idx === 0 ? (
                    <HiServer className="text-indigo-400" />
                  ) : (
